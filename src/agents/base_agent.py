...
class BaseAgent(ABC):
    ...
    def _log_incoming(self, state: SDLCState) -> None:
        ...
        if meta:
            ...
            task = meta.get('task', '?')
            self.logger.info(f'received from {from_a} task={task}')
    ...
    def _build_outgoing(self, merged_state: dict, updates: dict) -> Dict[str, Any]:
        to_agent = NEXT_AGENT.get(self.name, 'human')
        task = TASK_FOR_RECEIVER.get(to_agent, 'handoff')

        artifacts: Dict[str, Any] = {}
        for key in ('requirements', 'architecture', 'codebase', 'code_review',
